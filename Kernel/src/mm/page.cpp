...
Pageframe* pfn2page(u64 pfn) {
    ...
    return reinterpret_cast<Pageframe*>(siberix()->getMemory().getPageSect(sectionId).pages[offset]);
};
Pageframe*   addr2page(u64 address) { return pfn2page(address >> 12); }
PageSection* addr2sect(u64 address) { return &(siberix()->getMemory().getPageSect(address >> 30)); }