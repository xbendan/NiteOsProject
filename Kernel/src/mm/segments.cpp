...
Pageframe* SegAlloc::allocatePhysMemory4KPages(u64 amount) {
...
u64 SegAlloc::allocatePhysMemory4K(u64 amount) {
    u64 address = 0;
    int i       = 0;
    while (!address && i < 256) {
        PageBlock& block = siberix()->getMemory().getPageBlock(i);
        if (block.type == BlkTypeAvailable && block.end - block.start > (amount * PAGE_SIZE_4K)) {
            address      = block.start;
