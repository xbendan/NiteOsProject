...
namespace Memory {
    u16 blockSize[] = {
        8, 16, 24, 32, 48, 64, 96, 128, 192, 256, 512, 768, 1024, 1536, 2048, 4096
    };

    SlabAlloc::SlabAlloc() {
        for (int i = 0; i < SLAB_MAX_BLOCK_ORDER; i++) caches[i] = new SlabCache(blockSize[i], 0);
    }

    SlabAlloc::~SlabAlloc() {}

    SlabCache* SlabAlloc::getCache(u32 size) {
        SlabCache* cache;
    ...
    u64 SlabAlloc::alloc(u64 size) {
        ...
        if (++page->slabInuse == page->slabObjects) cpu->page = cache->request4KPage(&address);
