...
static inline pagedir_t   kPageDirs __attribute__((aligned(PAGE_SIZE_4K)));
static inline pagedir_t   kHeapDirs __attribute__((aligned(PAGE_SIZE_4K)));
static inline pagedir_t   kIoDirs[4] __attribute__((aligned(PAGE_SIZE_4K)));
static inline pagetable_t kHeapTables[TABLES_PER_DIR]
...
void
X64AddressSpace::map(u64 phys, u64 virt, u64 amount)
{
}

bool
X64AddressSpace::isPagePresent(u64 address)
{
    return false;
}

u64
X64AddressSpace::convertVirtToPhys(u64 address)
{
    return 0;
}
            ...
            if (counter >= amount) {
                ...
                while (counter--) {
                    ...
                    setPageFrame(
                      ...
                      (0 + ((pageDirOffset * PAGES_PER_TABLE) + offset) *
                             PAGE_SIZE_4K),
