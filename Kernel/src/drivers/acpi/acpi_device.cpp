    ...
    : m_name("ACPI Power Management")
{
    ...
    {
        case 0: rsdt = reinterpret_cast<AcpiRsdt*>(getIOMapping(rsdp->rsdtAddress)) break;
        case 2:
            rsdt = reinterpret_cast<AcpiRsdt*>(getIOMapping(rsdp->rsdtAddress));
            xsdt = reinterpret_cast<AcpiXsdt*>(getIOMapping(xsdp->xsdtAddress));
            break;
        default: Logger::getLogger("acpi").error("Invalid ACPI revision number %u", rsdp->revision); break;
    }
...
AcpiPmDevice::findTable(const char* str, int index) {
    if (!memcmp("DSDT", str, 4))
        return reinterpret_cast<T>(fade->dsdt);
