...
namespace Memory
{
    ...
    uintptr_t SlabAllocator::Allocate(size_t size)
    {
        ...
        {
            return KernelAllocate4KPages(ALIGN_UP(size, ARCH_PAGE_SIZE) / ARCH_PAGE_SIZE);
        }
        ...
        if (!page || page->slab_inuse >= page->slab_objects)
            page = cpu->page = this->Request4KPage(cache);
