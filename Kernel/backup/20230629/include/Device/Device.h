...
public:
    ...
    static SizedArrayList<LinkedList<Device*>,
                          DeviceType::DeviceTypeUnknown + 1>
      g_DeviceLists;

    Device(const char* name, DeviceBus bus, DeviceType type);
    Device(const char* name)
      : Device(name, DeviceBus::DeviceBusUnknown, DeviceType::DeviceTypeUnknown)
    {
    }
    Device(DeviceBus bus, DeviceType type)
      : Device(nullptr, bus, type)
    {
    }
    ...
    inline bool IsPropertySet(DeviceProperty property)
    {
        return m_Properties & property;
    }
    inline bool IsDependent(Device* device)
    {
