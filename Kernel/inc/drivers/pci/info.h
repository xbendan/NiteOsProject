...
namespace PCI {
    ...
    public:
        ...
        {
            return _vendorID ? _vendorID
                             : (_vendorID = read<uint16_t>(Regs::VendorID));
        }

        uint16_t getDeviceID()
        {
            return _deviceID ? _deviceID
                             : (_deviceID = read<uint16_t>(Regs::DeviceID));
        }

        uint8_t getClass()
        {
            return _class ? _class : (_class = read<uint8_t>(Regs::Class));
        }

        uint8_t getSubclass()
        {
            return _subclass ? _subclass
                             : (_subclass = read<uint8_t>(Regs::Subclass));
        }

        uint8_t getProgIF()
        {
            return _progIF ? _progIF : (_progIF = read<uint8_t>(Regs::ProgIF));
        }
